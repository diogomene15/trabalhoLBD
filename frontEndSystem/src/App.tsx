...
function App() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const nomeIngrediente = (formRef.current?.elements.namedItem('nome') as HTMLInputElement)?.value
    const descricaoIngrediente = (formRef.current?.elements.namedItem('descricao') as HTMLInputElement)?.value
    ...
    setIngredientes([...ingredientes, {nome: nomeIngrediente, descricao: descricaoIngrediente}])
  }
  return (<div className='app'>
    <div className="page-paper-container signup-container">
        ...
        <form id='create-ingrediente-form' ref={formRef} onSubmit={handleSubmit}>
            <input id="nome" className='signup-input' type="text" placeholder="*Nome" required />
            <textarea id="descricao" className='signup-input' placeholder="Descrição" required />
          ...
          <IngredienteItem
            ...
            onExcluir={async() => {
              if(ingrediente && ingrediente.id){
                ...
                const resDelete = await deleteIngrediente(ingrediente.id)
                if(resDelete.status < 400)
                  setIngredientes(ingredientes.filter((ingredienteItem) => ingredienteItem.id !== ingrediente.id))
              }
            }}
          />
        ))}
